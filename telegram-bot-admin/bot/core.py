# telegram-bot-admin/bot/core.py

from telegram.ext import Updater, CommandHandler, MessageHandler, Filters, ConversationHandler
from config import settings
# O'ZGARTIRILDI: Importlar aniqlashtirildi
...
def run_bot():
    ...
    upload_students_conv = ConversationHandler(
        entry_points=[MessageHandler(Filters.regex('^📤 O\'quvchilarni import qilish'), handlers.upload_students_start)],
        states={
            states.PROCESSING_EXCEL_FILE: [MessageHandler(Filters.document.mime_type("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"), handlers.process_excel_file)],
        },
        fallbacks=common_fallbacks,
    )
